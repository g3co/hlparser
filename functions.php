...
function makeUrl ($path,$alias){
    if(QueryGet("SELECT url_alias_id FROM oc_url_alias WHERE keyword = '".$alias."'")){ $alias .= 'db'; }
...
function getId ($str){
	...
	if(isset($request[0])) return $request[0]['id'];
	else return false;
}
...
function getCatArr ($str,$level){
	...
	if(isset($request[0])) return $request[0];
	else return false;
}
...
function makeCategory ($id,$parent,$name,$target){
	...
	*/
	...
	$sql = "INSERT INTO oc_category (`category_id`, `image`, `parent_id`, `top`, `column`, `sort_order`, `status`, `date_added`, `date_modified`) "
                . "VALUES ({$id}, '', {$parent}, {$top}, '1', '1', '".$catOnOff."', '".date("Y-m-d H:i:s")."', '".date("Y-m-d H:i:s")."')";
	...
	*/
	$sql = "INSERT INTO oc_category_description (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, "
                . "`seo_title`, `seo_h1`) VALUES ({$id}, '1', '{$name}', '', '', '', '', '')";
	QueryExec($sql);
...
function makeManufacturer ($id,$name){
	...
	QueryExec("INSERT INTO oc_manufacturer_description (`manufacturer_id`, `language_id`, `description`, `meta_description`, `meta_keyword`, `seo_title`, `seo_h1`) "
                . "VALUES (".$id.", '1', '', '', '', '', '')");
